...
%             Wu =-gamma/2/pi*0.7889*[x/r^2*phi_zu y/r^2*phi_zu -1/r*phi_ru]';
            Wu =-gamma/2/pi*0.7889*[xn/rn^2*phi_zu yn/rn^2*phi_zu -1/rn*phi_ru]'; % n点速度应当使用n点的坐标而不是原来位置的坐标
        end
    end
end
...
function [phi_r,phi_z]=STRMFN(r_cal,z_cal,R0)
            ...
            % k = (r1 - r2) / (r1 + r2);
            % phi = (k)^2 / (0.25 + 0.75 * sqrt(1 - (k)^2)) * (r1 + r2);
            % phi_k =...
            %     2 * k / (0.25 + 0.75 * sqrt(1 - (k)^2)) +...
            %     0.75 * pow(k, 3) / (sqrt(1 - (k)^2)) / (0.25 + 0.75 * (1 - (k)^2));
            % k_r1 = 2 * r2 / (r1 + r2)^2;
            % k_r2 = -2 * r1 / (r1 + r2)^2;

            % phi_r =...
            %     phi_k * (k_r1 * r1_r + k_r2 * r2_r) + phi / (r1 + r2) * (r1_r + r2_r);
            % phi_z =...
            %     phi_k * (k_r1 * r1_z + k_r2 * r2_z) + phi / (r1 + r2) * (r1_z + r2_z);
            
            phi_r1 = ((-r1 + r2) *(-((0.75 *r2^2 *sqrt((r1 *r2)/(r1 + r2)^2))/r1) + ...
                r2 *(-0.75 - 3. *sqrt((r1 *r2)/(r1 + r2)^2)) + ...
                r1 *(-0.25 - 2.25 *sqrt((r1 *r2)/(r1 + r2)^2))))/((r1 + ...
                r2)^2 *(0.25 + 1.5 *sqrt((r1 *r2)/(r1 + r2)^2))^2);
            phi_r2 = ((-r1 + r2)* ((0.75 *r1^2 *sqrt((r1 *r2)/(r1 + r2)^2))/r2 + ...
                r2 *(0.25 + 2.25 *sqrt((r1 * r2)/(r1 + r2)^2)) + ...
                r1 *(0.75 + 3. *sqrt((r1 * r2)/(r1 + r2)^2))))/((r1 + r2)^2 *(0.25 + ...
                1.5 *sqrt((r1 * r2)/(r1 + r2)^2))^2);
